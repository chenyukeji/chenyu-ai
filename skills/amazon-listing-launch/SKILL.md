...
description: Independently create Amazon Listing drafts, localized content, a seven-image brief, upload workbook, compliance bundle, and pre-launch checks from operations-approved product facts. A prior Product Decision Card is optional; do not use this skill to decide whether to develop a product or to publish without explicit authorization.
---
...
4. Save localized content in one JSON object matching `examples/listings.json`, then run:
   ...
   `python skills/amazon-listing-launch/scripts/run.py --product-master <master.json> --listings <listings.json> --output-dir <folder>`
