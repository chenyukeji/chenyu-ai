...
- After changing a plugin, bump its plain `major.minor.patch` version in both `plugin.json` and `.codex-plugin/plugin.json`, validate the affected Skill and plugin, run relevant tests, commit the source change, and push it to GitHub before considering the work complete. The website displays the version with a leading `v`; do not add Codex timestamp suffixes.
- Keep the two plugin manifest versions equal; Codex reads the root manifest when choosing an installed cache path.
...
- The server timer in `deploy/chenyu-plugin-sync.timer` checks GitHub `main` every day at local midnight and runs `scripts/sync_remote_plugins.py`. It validates new manifests before a fast-forward, refreshes installed caches, and leaves dirty or divergent checkouts untouched. The website reads the checkout directly. At 00:10 Asia/Shanghai, an open admin page calls the same server sync workflow and reuses the midnight result if it already succeeded; other pages refresh displayed capabilities once daily.
