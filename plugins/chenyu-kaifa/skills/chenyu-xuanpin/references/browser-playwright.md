...
3. 使用卖家精灵“查竞品”按站点和 ASIN 补充标题、上架时间、Review、售价、BSR、预估月销量、品类、商品链接和图片。每个站点开始查询前必须切换站点，并回读控件确认实际值为“美国站”或“德国站”；每次单条或批量查询前，确认“展示所有变体”未勾选。控件缺失或状态无法确认时停止补数，不能把变体结果或无结果归因于目标 ASIN。批量查询通过页面 ASIN 输入框提交多个 ASIN，不依赖直接打开查询结果 URL。
...
J 使用卖家精灵“选产品”页面，而非 Amazon 新品榜。按站点选择“近60天”上架、“FBM”配送、最低预估月销量后回读页面控件和查询参数，保存原始结果、采集页与筛选条件；随后以记录中的实际日期、配送方式和销量再次核验。若页面是游客状态且环境变量中有账号密码，先在同一会话登录并核验身份；登录/验证码受限时停止并保留阻塞原因。未提供凭据的游客可见结果或分页截断应标记部分覆盖，不声称全站穷尽。站外流量和功能创新不从标题、价格或销量推断。
