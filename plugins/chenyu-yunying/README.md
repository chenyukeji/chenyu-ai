...
Listing 描述统一为“概述、3—5条特征、产品参数、包装内容”，默认纯文本。附带三个 Python 3.10+ 标准库脚本：提取 XLSX 证据、按链接抓取竞品 Listing、检查关键词/文案。爬虫合并重复 ASIN 来源，提取标题、五点、普通/A+描述、选中变体，并下载对应高清产品图册及 A+ 图片，保存来源快照和逐图状态；静态页面未读全时由可用浏览器补读，不绕过验证码。文案由模型结合自有事实生成；没有后台任务或自动发布服务。读取失败和未确认事实会在审核说明中披露。
