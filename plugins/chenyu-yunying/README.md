...
Listing 描述统一为“概述、3—5条特征、产品参数、包装内容”，默认纯文本。附带两个 Python 3.10+ 标准库脚本，分别提取 XLSX 证据与检查关键词/文案。竞品页面由当前助手的网页或浏览器工具读取，文案由模型生成；没有独立爬虫、后台任务或自动发布服务。读取失败和未确认事实会在审核说明中披露。
