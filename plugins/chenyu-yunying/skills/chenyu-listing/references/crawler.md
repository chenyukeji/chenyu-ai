...
- title、bullets、description，单独的 aplus_text，页面明确显示的 selected_variant。
- status：complete 表示标题、五点区域和普通描述均有文本；partial 表示有标题但字段不全；failed 表示请求失败、验证码、产品身份冲突或没有产品标题。complete 不意味着五点恰好五条、动态模块完整或文案已审核。
- field_status 的 not_found_in_html 表示未从静态 HTML 读到，不能写成“商品没有描述”。failure_reason、http_status、warnings 和 snapshot 便于追溯。

进程退出码 0 表示至少一条完整或部分读取；2 表示全部失败或没有可抓取商品，先检查 JSON 详情。不要把失败网页、搜索摘要、A+ 文本或推测文案填成完整的普通描述。浏览器补读须记录新来源、时间和读取方式；仍失败则披露缺口，依靠自有事实生成草稿。
