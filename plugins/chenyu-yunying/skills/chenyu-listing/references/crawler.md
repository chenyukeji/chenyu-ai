...
- 下载当前变体静态 HTML 明确提供的高清主图、图册、普通描述图片、A+ 和可持续模块图片。明确不抓品牌故事模块。只访问 Amazon 图片 CDN，单图最多 25 MB，最多记录 100 个页面候选；过滤跟踪像素及链接到其他 ASIN 的交叉销售卡片，不混入推荐商品、轮播广告或其他变体图片。
...
- images 为逐图证据：roles 区分 main、gallery、description、aplus、sustainability；记录原 URL、最终 URL、本地路径、MIME、字节数、SHA-256 及可识别的像素尺寸。相同 URL 合并角色，相同文件哈希只保存一次并标记 duplicate。
